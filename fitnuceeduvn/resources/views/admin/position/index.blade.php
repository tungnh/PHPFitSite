    ...
    <section class="vbox">
        <section class="scrollable padder">
            <ul class="breadcrumb no-border no-radius b-b b-light pull-in">
                ...
                <li>
                    <a href="{{ url('/admin/position/index') }}">Chức vụ</a>
                </li>
            </ul>
            ...
            <form class="bs-example form-horizontal" name="form" method="GET" action="{{ url('/admin/position/index') }}">
                            ...
                            <div class="panel-body">
                                <div class="form-group m-b-sm">
                                    <div class="col-lg-9">
                                        ...
                                        <div class="col-lg-9">
                                            <input id="keysearch" type="text" value="{{ $s }}" class="form-control" placeholder="Tên chức vụ, mô tả..." name="s">
                                        </div>
                                    </div>
                                    <div class="col-lg-3">
            ...
            <section class="panel panel-default">
                <div class="table-responsive">
                    <table class="table table-striped m-b-none">
                        ...
                        <tbody>
                            <?php $index = ($positions->currentPage() - 1) * Constants::$PAGE_NUMBER + 1; ?>
                            @foreach($positions as $position)
                            <tr>
                                <td align="center">{{ $index }}</td>
                                <td>{{ $position->position_name }}</td>
                                <td align="center">{{ $position->description }}</td>
                                <td align="center">{{ $position->order_number }}</td>
                                <td width="40px;" class="text-center" style="vertical-align: middle;">
                                    <a href="{{ url('/admin/position/edit/'.$position->id) }}"><i class="fa fa-edit text-dark text"></i></a>
                                </td>
                                <td width="40px;" class="text-center" style="vertical-align: middle;">
                                    <form id="delete-form-{{ $position->id }}" action="{{ url('/admin/position/delete/'.$position->id) }}" method="POST" style="display: none;">
                                        {{ csrf_field() }}
                                    </form>
                                    <a href="javascript: void(0);" onclick="event.preventDefault(); if (confirm('Bạn có chắc chắn muốn xóa bản ghi này!')) document.getElementById('delete-form-{{ $position->id }}').submit();"><i class="fa fa-times text-danger text"></i></a>
                                </td>
                            </tr>
                            <?php $index++; ?>
                            @endforeach
                    ...
                    <div class="row text-center-xs">
                        <div class="col-md-4 hidden-sm">
                            <p class="m-t" style="color: #2e3e4e;">
                                Tổng số <strong>{{ $positions->total() }}</strong> dữ liệu
                            </p>
                        </div>
                        <!--                            <div class="col-md-4 hidden-sm">
